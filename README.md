...
* [Clafer](https://github.com/gsdlab/clafer) v0.4.5
...
Clafer is now available on [Hackage](http://hackage.haskell.org/package/claferIG-0.4.5/) and it can be installed using either [`stack`](https://github.com/commercialhaskell/stack) or [`cabal-install`](https://hackage.haskell.org/package/cabal-install).
...
* [GHC](https://www.haskell.org/downloads) >= 7.10.3 and 8.0.1 are recommended,
* `cabal-install` >= 1.18, should be installed together with a GHC distribution,

1. Install GHC
2. `cabal update`
3. `cabal install claferIG`
4. `cd <cabal's lib or share folder>`  (`C:\Users\<user>\AppData\Roaming\cabal\x86_64-windows-ghc-8.0.1\claferIG-0.4.5` on Windows or `.cabal/share/x86_64-linux-ghc-8.0.1/claferIG-0.4.5/` on Linux)
