...
* [Clafer](https://github.com/gsdlab/clafer) v0.5.0
...
Clafer is now available on [Hackage](http://hackage.haskell.org/package/claferIG-0.5.0/) and it can be installed using either [`stack`](https://github.com/commercialhaskell/stack) or [`cabal-install`](https://hackage.haskell.org/package/cabal-install).
...
* [GHC](https://www.haskell.org/downloads) >= 7.10.3 and 8.0.2 are recommended,
* `cabal-install` >= 1.18, should be installed together with a GHC distribution,

1. Install GHC
2. `cabal update`
3. `cabal install claferIG`
4. `cd <cabal's lib or share folder>`  (`C:\Users\<user>\AppData\Roaming\cabal\x86_64-windows-ghc-8.0.2\claferIG-0.5.0` on Windows or `.cabal/share/x86_64-linux-ghc-8.0.2/claferIG-0.5.0/` on Linux)
...
* update MSYS2 packages
   * follow [III. Updating packages](http://sourceforge.net/p/msys2/wiki/MSYS2%20installation/) but always execute `stack exec pacman -- <arguments for pacman>`
* execute
